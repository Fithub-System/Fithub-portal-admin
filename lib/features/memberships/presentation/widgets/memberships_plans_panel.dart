...
/// Plans list + create / deactivate / assign (FEAT-07), embedded under Members.
class MembershipsPlansPanel extends StatefulWidget {
  const MembershipsPlansPanel({super.key, required this.canWrite});

  final bool canWrite;

  @override
  State<MembershipsPlansPanel> createState() => _MembershipsPlansPanelState();

  static Future<void> showAssignSheet(
    BuildContext context, {
    String? initialAthleteId,
  }) async {
    final cubit = context.read<MembershipsCubit>();
    final state = cubit.state;
    if (state.status != MembershipsStatus.ready) {
      await cubit.load();
    }
    if (!context.mounted) return;
    await _MembershipsPlansPanelState.openAssignSheet(
      context,
      context.read<MembershipsCubit>().state,
      initialAthleteId: initialAthleteId,
    );
  }
}

class _MembershipsPlansPanelState extends State<MembershipsPlansPanel> {
  @override
  void initState() {
    super.initState();
    final cubit = context.read<MembershipsCubit>();
    if (cubit.state.status == MembershipsStatus.initial) {
      cubit.load();
    }
  }
  ...
  static Future<void> openAssignSheet(
    BuildContext context,
    ...
    var selectedAthleteId = initialAthleteId ?? state.athletes.first.id;
    if (!state.athletes.any((a) => a.id == selectedAthleteId)) {
      selectedAthleteId = state.athletes.first.id;
    }
