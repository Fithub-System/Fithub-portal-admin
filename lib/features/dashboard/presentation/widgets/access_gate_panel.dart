...
/// Stitch Access Gate 1 + Access Granted stack (Overview col-span-4).
///
/// Tap / CTA opens FEAT-12 Check-in Gate focus (not a rail tab).
/// §4.1: Access Granted sample chrome (John Smith) always ships when no live
/// rejection is pending; live approved scan replaces the sample member.
class AccessGatePanel extends StatelessWidget {
  ...
  /// When true (default), shows Stitch Access Granted sample if no reject.
  final bool showGrantedFixture;

  @override
  Widget build(BuildContext context) {
    final textTheme = Theme.of(context).textTheme;
    final hasReject = lastScanRejectReason != null &&
        lastScanRejectReason!.isNotEmpty;
    final liveGranted = lastScanApproved &&
        (lastScanMemberName?.isNotEmpty ?? false);
    final grantedName = liveGranted
        ? lastScanMemberName!
        : OverviewStitchFixtures.grantedMemberName;
    final grantedId = liveGranted
        ? null
        : OverviewStitchFixtures.grantedMemberId;
    final showGranted = !hasReject && (liveGranted || showGrantedFixture);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (memberId != null) ...[
                      ...
                      Text(
                        'dashboard.access_gate.member_id'.tr(
