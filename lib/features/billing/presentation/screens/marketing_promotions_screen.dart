...
/// Marketing & Promotions — Billing section (FEAT-08 US-B).
///
/// Stitch: `c3207a6938bf40a7872dde7532020ef9` (Marketing & Promotions).
/// Design Gap: full promos/campaigns UI not in v1 — Billing list only.
class MarketingPromotionsScreen extends StatefulWidget {
  const MarketingPromotionsScreen({super.key, required this.canWrite});

  static const String stitchScreenId = 'c3207a6938bf40a7872dde7532020ef9';
  static const String stitchScreenTitle = 'Marketing & Promotions';

  /// Admin-only mark paid/waived/failed + apply freeze (AC-B1 / AC-C3).
  final bool canWrite;

  @override
  State<MarketingPromotionsScreen> createState() =>
      _MarketingPromotionsScreenState();
}

class _MarketingPromotionsScreenState extends State<MarketingPromotionsScreen> {
  @override
  void initState() {
    super.initState();
                ...
                if (state.charges.isEmpty)
                  ...
                  ...state.charges.map(
                    (charge) => _ChargeTile(
                      ...
                      onMarkWaived: () =>
                          context.read<BillingCubit>().markStatus(
                            chargeId: charge.id,
